...
const dragGraph = function({
  ...
  if (type === 'text') {
    ...
    this.x = x + halfWidth + 140;
    console.log(this)
    this.y = y + halfHeight + 100;
  } else {
    this.x = x;
    this.y = y;
  }
...
dragGraph.prototype = {
  ...
  paint() {
    ...
    if (this.type === 'text') {
      ...
      this.ctx.setStrokeStyle('rgb(240,240,240)');
      this.ctx.lineDashOffset = 10;

      const textWidth = this.ctx.measureText(this.text).width + 16;
      const textHeight = this.fontSize + 16
      const halfWidth = textWidth / 2;
      const halfHeight = textHeight / 2;
      const textX = this.x - halfWidth;
      const textY = this.y - halfHeight;
      this.ctx.drawImage(this.textBgUrl, textX, textY, textWidth, textHeight);
    }
  ...
  methods: {
    ...
    draw() {
      if (this.data.bgImage !== '') {
        this.ctx.drawImage(this.data.bgImage, this.data.draw_start_width, this.data.draw_start_height, this.toPx(this.data.bg_width), this.toPx(this.data.bg_hieght));
      }
    ...
    changeBgImage(imageUrl, draw_start_width, draw_start_height, bg_width, bg_hieght) {
      this.data.bgColor = '';
      this.data.bgImage = imageUrl;
      this.data.draw_start_width = draw_start_width
      this.data.draw_start_height = draw_start_height
      this.data.bg_width = bg_width
      this.data.bg_hieght = bg_hieght
      this.draw();
    },
    getSelect() {
      const drawArr = this.drawArr;
      console.log(drawArr)
      return drawArr;
    },
    select() {
